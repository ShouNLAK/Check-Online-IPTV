...
{
    const StreamInfo *s1 = a;
    const StreamInfo *s2 = b;
...
void Check_Active(const char* input, const char* output) 
{
    ...
    while (fgets(line, sizeof(line), fi)) {
        ...
        boolean is_ts = FALSE; // Flag to check if the URL is a .ts file
        do {
            if (Test)
                firstTry = 0;  // Not the first try anymore

            is_timeout = 0;
            int status_code = 0, got_response = 0, is_download = 0;
            char resp[256];

            if (strstr(url, "http://") || strstr(url, "https://")) {
                snprintf(command, sizeof(command),
                        "curl -I -s --max-time 23 \"%s\" | find \"HTTP/\" 2>&1", url);
                ...
                { 
                    char newUrl[512] = {0}; 
                    snprintf(command, sizeof(command), "curl -I -s \"%s\" | find \"Location:\" 2>&1", url); 
                    fp = _popen(command, "r"); 
                    if (fp) {
                        while (fgets(resp, sizeof(resp), fp)) {
                            if (strstr(resp, "Location:")) {
                                ...
                                // Run command curl -I on the new URL to check its response
                                char check_cmd[512] = {0};
                                char check_resp[1024] = {0};
                                snprintf(check_cmd, sizeof(check_cmd), "curl -I -s \"%s\"", newUrl);
                                FILE *check_fp = _popen(check_cmd, "r");
                                if (check_fp) {
                                    // If any response is obtained, consider the new URL active
                                    if (fgets(check_resp, sizeof(check_resp), check_fp)) {
                                        is_ts = TRUE;
                                        is_active = 1;  // count as active since valid response received
                                    }
                                    _pclose(check_fp);
                                }
                                break;  // URL found & processed, break out of while loop
                            }
                        }
                        _pclose(fp);
                    }
                ...
                if (is_timeout) {
                ...
                else if ((!is_ts && (got_response && status_code >= 200 && status_code < 400)) || is_download) {
                    StreamInfo streams[100] = {0};
                    int stream_count = 0;
                    char playlist_cmd[1024] = {0};
                    snprintf(playlist_cmd, sizeof(playlist_cmd), "curl -s -- \"%s\"", url);
                    FILE *fp_playlist = _popen(playlist_cmd, "r");
                    if (fp_playlist == NULL) {
                        perror("Failed to open pipe for playlist");
                        return 1;
                    }
                    char playlist_line[512] = {0};
                    ...
                    while (fgets(playlist_line, sizeof(playlist_line), fp_playlist) != NULL) {
                        ...
                        if (res_ptr) {
                            ...
                            int i = 0;
                            while (res_ptr[i] && res_ptr[i] != ',' && i < (int)(sizeof(tmp)-1)) {
                                tmp[i] = res_ptr[i];
                                i++;
                            }
                            tmp[i] = '\0';
                            strncpy(last_resolution, tmp, sizeof(last_resolution)-1);
                        }
                        // If we detected a resolution and then find the media URL, store it.
                        else if (strlen(last_resolution) > 0) {
                            if (stream_count < 100) {
                                strncpy(streams[stream_count].resolution, last_resolution,
                                        sizeof(streams[stream_count].resolution)-1);
                                strncpy(streams[stream_count].stream_url, playlist_line,
                                        sizeof(streams[stream_count].stream_url)-1);
                                int w = 0, h = 0;
                    ...
                    // New logic: If no stream with resolution info was found, assume the URL is auto-updated best quality.
                    if (stream_count == 0 && pass) {
                        ...
                        for (int i = 0; i < stream_count; i++) {
                            char candidate_url[512] = {0};
                            // If the candidate doesn't start with "http", treat it as relative.
                            ...
                            snprintf(test_cmd, sizeof(test_cmd),
                                    "curl -I -s --max-time 15 \"%s\" | find \"HTTP/\" 2>&1",
                                    candidate_url);
                            FILE *fp_test = _popen(test_cmd, "r");
                            if (fp_test) {
                                while (fgets(test_resp, sizeof(test_resp), fp_test)) {
                                    if (strncmp(test_resp, "HTTP/", 5) == 0) {
                                        char *token = strtok(test_resp, " ");
                                        token = strtok(NULL, " ");
                            ...
                            if (candidate_active) {
                                strncpy(fetched_resolution, streams[i].resolution, sizeof(fetched_resolution)-1);
                                fetched_resolution[sizeof(fetched_resolution)-1] = '\0';
                                strncpy(url, candidate_url, strlen(candidate_url)+1);
                                candidate_found = 1;
                                break;
                            }
                        }
    ...
    // Print the summary statistics with color.
